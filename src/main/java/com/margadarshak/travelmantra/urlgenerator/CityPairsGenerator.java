...
public class CityPairsGenerator {
	...
	static Logger logger = Logger.getLogger(CityPairsGenerator.class.getName());
	...
	private List<CityPair> generateCityPairs() throws IOException, JSONException {
		...
		if (file.exists()) {
			...
			for (int i = 0; i < jarr.length(); i++) {
				...
				logger.info("list of city pairs: " + cityPairList.size());
			}
		}
		logger.info("Size of listOf Cities is :" + cityPairList.size());
		logger.info("Size of flight code map is :" + cityCodesForFlight.size());
		logger.info("Size of rail code map is :" + cityCodesForTrain.size());
		logger.info("Size of bus code map is :" + cityCodesForBus.size());
